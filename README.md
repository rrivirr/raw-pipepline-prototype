...
## Overview
Hardware sensors measure fundamental physical parameters, such as voltage, current, and time.  These are then converted into a meaningful values for decision making and discovery: gas concentrations, energy transmission rates, flow volumes, etc.  To report accurate measurements, sophisticated mathematical conversions run on the sensor hardware, including various corrections to the low level measurement and calibration algorithms converting raw measurements into the parameter of interest.  These transofmrations are rarely exposed to data consumers, the adjustments process is obscure, and historical updates to corrective parameters are typically untracked.  This significantly reduces data reliability, generating extensive re-study and re-processing by scientific data users, or unknown inaccuracies in data.

This project presents a proof of concept for moving low level data processing to the cloud.  Rather than tuning and applying measurement correction and calibration parameters in sensor hardware, these values are stored and applied in the cloud.  The sensor device retains responsibility for producing reliable fundamental physical measurements, while the cloud converts these into the environmental parameter of interest.  This architecture has several advantages.  First, conversion algorithms can be developed, optimized, and re-run iteratively on data without requiring device firmware updates.  Second, correction and calibration control parameters can be updated and tracked in a device specific data processing lineage.  Third, device re-calibrations can be tracked historically, viewed, modified, and re-run as well.  And fourth, the correction and calibration pipelines themselves can be re-ordered and improved over time, again without requiring time consuming field visits to deployed sensors.

## Technical Summary
This project is built to consume data from the existing RRIV open source environmental sensor device inventory and telemetry platform, which ingests data into TimescaleDB.  Data is synced through Iceberg tables to Snowflake for pipeline processing.  Processing runs through a DAG in Snowflake, with control parameters for each step stored in a lineage tracking table that is later exported back to TimescaleDB along with the corrected and calibrated data for service.  The pipeline runs automated every hour, or can be triggered for arbitrary date range rerun to process changes in control parameters.  Control parameters are updated through database tables.  Data outputs are synced back to TimscaleDB via S3 and patch overwrite data from previous runs in the official data service tables. 

### Data Flow
Data originates from hypertables in TimescaleDB and is made available to SnowFlake via TigerCloud's Iceberg sync destination connector, using AWS S3 tables as a shared storage backend.  After processing, data is written in incremental parquet files to and an S3 bucket stage, organized as `$sensor_serial_number/YYYY/MM/DD/HH/$processing_lineage/$filname.parquet`.  As new S3 files arrive in this bucket, TigerCloud's S3 source connector upserts data by primary key, ensuring the official data set contains only data from the most recent pipeline run for every date range.  Data scientists may access to the full S3 archive to recreate the processing outputs at any point in history.
