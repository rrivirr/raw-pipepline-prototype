...
CREATE OR REPLACE PROCEDURE export_calibrated_data(staged_file VARCHAR(255))
RETURNS BOOL
LANGUAGE SQL
AS
$$
...
BEGIN
  ...
  file_path := 's3://rriv-corrected-raw/calibrated-data-export/'
               || TO_VARCHAR(CURRENT_TIMESTAMP(), 'YYYY-MM-DD"T"HH24-MI-SS')
               || '_calibrated_data.parquet';
